...
export async function getTemperature(): Promise<number | undefined> {
    try {
        ...
        if (platform === 'linux') {
            ...
            console.error('Temperature monitoring is not natively supported on macOS.');
            return undefined;
        } else if (platform === 'win32') {
            const output = execSync('wmic /namespace:\\\\root\\wmi PATH MSAcpi_ThermalZoneTemperature get CurrentTemperature').toString();
 ...
 */
export const trackRequestResponseTime = (req: Request, res: Response, next: NextFunction) => {
...
export const systemMonitor = (options: MonitorOptions = defaultOptions) => {
    return async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (options.logs) data.logs = await getLogs(options.logs.path, options.logs.keyword);
